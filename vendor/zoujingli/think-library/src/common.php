...
if (!function_exists('format_date')) {
     ...
     * @return false|string
     */
    function format_date($datetime, $format = 'Y-m')
    {
        if (empty($datetime)) return '-';
        if (is_numeric($datetime)) {
            return date($format, $datetime);
...
if (PHP_SAPI !== 'cli') {
    ...
    Middleware::add(function (Request $request, \Closure $next, $header = []) {
        if (($origin = $request->header('origin', '*')) !== '*') {
            ...
            $header['Access-Control-Allow-Headers'] = 'Authorization,Content-Type,If-Match,If-Modified-Since,If-None-Match,If-Unmodified-Since,X-Requested-With,Accept, Connection, User-Agent, Cookie,token';
...
if (!function_exists('shoujihao')) {
    /**
     * 用户手机号
     * @param string $datetime 输入用户id
     * @return false|string
     */
    function shoujihao($datetime)
    {
        if (empty($datetime)) return '-';
        $phone = Db::name('store_member')->where(['id'=>$datetime])->value('phone');
        return $phone;
    }
}

if (!function_exists('zhuangtai')) {
    /**
     * 机具状态
     * @param string $datetime 输入id
     * @return false|string
     */
    function zhuangtai($datetime)
    {
        ...
        return $zhuangtai[$datetime];
    }
}

if (!function_exists('memberlaver')) {
    /**
     * 机具状态
     * @param string $datetime 输入id
     * @return false|string
     */
    function memberlaver($datetime)
    {
        if (empty($datetime)) return '-';
        $phone = Db::name('store_member_laver')->where(['id'=>$datetime])->value('name');
        return $phone;
    }
}

if (!function_exists('manuallaver')) {
    /**
     * 机具状态
     * @param string $datetime 输入id
     * @return false|string
     */
    function manuallaver($datetime)
    {
        if (empty($datetime)) return '-';
        $phone = Db::name('store_manual_laver')->where(['id'=>$datetime])->value('name');
        return $phone;
    }
}

if (!function_exists('provinceid')) {
    /**
     * 机具状态
     * @param string $datetime 输入id
     * @return false|string
     */
    function provinceid($datetime)
    {
        if (empty($datetime)) return '-';
        $phone = Db::name('member_city')->where(['id'=>$datetime])->value('area_name');
        return $phone;
    }
}
